...
/**
 * Abstract class to hold a key->value mapping for a uniform name to a specific
 * material property. This internal to the package and is only used by the
...
abstract class MaterialProperty {
	...
	protected static class TextureProperty extends MaterialProperty {
		...
		/**
		 * Sets the texture property value to the passed in value. If the
		 * property reference is null, it creates it with the passed in uniform
		 * type, assigns the instance to the property reference, and sets the
		 * passed in value
		 * 
		 * @param propRef
		 * @param uniformType
	...
	protected static class Vector3fProperty extends MaterialProperty {
		 ...
		 * @param z
		 */
		public static Vector3fProperty set(Vector3fProperty propRef, UniformType uniformType, float x, float y,
				float z) {
			if (propRef == null)
