...
public class UniformData {
	...
	public void set(String uniformName, Vector3fc value) {
		...
		// If the previously set value is the same we don't set it again
		if (VectorUtils.compare((Vector3fc) uniform.value, value))
			return;
	...
	public void set(String uniformName, int value) {
		Uniform uniform = getUniform(uniformName);
		// If the previously set value is the same we don't set it again
		if (uniform.value != null && ((int) uniform.value) == value)
			return;
	...
	public void set(String uniformName, float value) {
		Uniform uniform = getUniform(uniformName);
		// If the previously set value is the same we don't set it again
		if (uniform.value != null && ((float) uniform.value) == value)
			return;
	...
	public void set(String uniformName, Matrix4fc value) {
		Uniform uniform = getUniform(uniformName);
		// If the previously set value is the same we don't set it again
		if (MatrixUtils.compare((Matrix4fc) uniform.value, value))
			return;
