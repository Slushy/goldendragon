...
import engine.graphics.MaterialProperty.FloatProperty;
import engine.graphics.MaterialProperty.TextureProperty;
import engine.graphics.MaterialProperty.Vector3fProperty;
 ...
 * be more performant unless you plan on changing a material's properties for
 * every game object using that material.
 * 
...
public class MaterialPropertyBlock {
	...
	/**
	 * Does a null check against the current texture and returns the
	 * texture value if not null, otherwise if it is null it will return
	 * the value of the passed in texture.
	 * 
	 * @param texture
	 *            default "fallback" texture to return if current
	 *            texture is null
	 ...
	 * @return current specular color if not null otherwise the fallback specular color
	 */
	 ...
	 * shininess factor value if not null, otherwise if it is null it will return
	 * the value of the passed in shininess factor.
	 * 
	 * @param color
	 *            default "fallback" shininess factor to return if current
	 *            shininess factor is null
	 * @return current shininess factor if not null otherwise the fallback shininess factor
	 */
