...
import engine.utils.MemoryUtils;
...
public class VAO implements IBindable {
	...
	public void bindInterleavedVBO(int vertexCount, VBO[] vbos, float[][] data) {
		...
		int vertexByteLength = totalSize * MemoryUtils.FLOAT_SIZE_BYTES;
		
		// Store the vbo as an attribute
		for (int i = 0, currOffset = 0; i < vbos.length; i++) {
			storeVBO(vbos[i], GL11.GL_FLOAT, vertexByteLength, (currOffset * MemoryUtils.FLOAT_SIZE_BYTES), (i+1 == vbos.length));
