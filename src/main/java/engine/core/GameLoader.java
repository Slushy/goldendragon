...
final class GameLoader {
	 ...
	 * 
	 * @param display
	 *            the window display to begin loading the splash on
	 ...
	 */
	public void load(Display display, Consumer<String> onLoadingComplete) {
		...
		if (appSplashLoader != null) {
			try {
				SceneManager.loadAndShowApplicationSplash(display, appSplashLoader);
			} catch (Exception e) {
				e.printStackTrace();
